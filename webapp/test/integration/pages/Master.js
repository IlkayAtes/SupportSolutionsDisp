				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("SolcatId") === oObject.SolcatId;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].SolcatId;
										} else {
											sCurrentId = aItemsNotInTheList[0].SolcatId;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("SOLCATSet", {
											SolcatId : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("SolcatId"),
							title: oBindingContext.getProperty("SolcatTxt")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "SOLCATSet (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'SOLCATSet (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("SolcatTxt");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all SOLCATSet to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "SOLCATSet (" + iExpectedLength + ")"}),
									success : function () {
